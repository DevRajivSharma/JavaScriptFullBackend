...
const createPlaylist = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.log(err);
        throw new ApiError(500, "Something went wrong while adding playlist to user");
    }
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    playlist.videos.push({
        videoId, position: position || (playlist.videos ?
...
const uploadVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    playlist.videos.push({
        videoId: video._id, position: position || playlist.videos ?
            playlist.videos.length : 0 + 1
    });
    await playlist.save();
...
const getAllUserPlaylist = asyncHandler(async (req, res) => {
    const userPlaylistId = req.user.playLists;
    if (!userPlaylistId) {
...
const toggleSavePlaylist = asyncHandler(async (req, res) => {
    ...
    const isSaved = await user.playLists.includes(playlist._id);
    if (isSaved) {
        user.playLists.pull(playlistId);
        await user.save();
        return res.status(200).json(
            new ApiResponse(200, "Playlist removed from saved list")
        )
    }
    user.playLists.push(playlistId);
    await user.save();
    return res.status(200).json(
        new ApiResponse(200, "Playlist added to saved list")
    )
